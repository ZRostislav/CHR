...
for (const country of countries) {
  const option = `
        <li class="option">
            ...
            <span class="codePhone" style="display: none;">${
              country.code
            }</span>
        </li> `;
...
// Получаем все элементы после вставки в DOM
options = document.querySelectorAll(".option");

function selectOption() {
  const icon = this.querySelector(".iconify").cloneNode(true);
  const phone_code = this.querySelector("strong").textContent; // Получаем номер из <strong>
  const countryCode = document.getElementById("codePhone").textContent; // Извлекаем код страны из div с id="codePhone"
  ...
  input_box.focus(); // Фокусируемся на поле
  ...
  // Логируем код страны
  console.log(countryCode); // Логируем код страны
}
...
// Навешиваем обработчики событий на все страны
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    // Получаем код страны из div с id="codePhone"
    let selectedCountryCodeElement = document.getElementById("codePhone");
    let countryCode = selectedCountryCodeElement
      ? selectedCountryCodeElement.textContent
      : ""; // Извлекаем код страны
    let phoneNumber = phoneInput.value.trim();

    // Проверяем, чтобы номер был не короче 8 цифр
    if (phoneNumber.length < 8) {
      alert("Номер телефона должен содержать минимум 8 цифр!");
      return;
    }

    // Собираем полный номер
    let fullNumber = `${countryCode} ${phoneNumber}`;
    let encodedNumber = encodeURIComponent(fullNumber);

    let googleFormUrl = `https://docs.google.com/forms/d/e/1FAIpQLSebLVeeGxycoSFf5msbpA1Pz-TVULP5eZYS1eEWX3mISirODA/formResponse?entry.1461931232=${encodedNumber}`;

    // Используем window.location.href для отправки данных в Google Forms
    window.location.href = googleFormUrl; // Перенаправляем пользователя
  });
});
